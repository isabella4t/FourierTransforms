...
public class Polynomial {
    ...
    public Complex[] FastFourierTransform(int n, int k){
        Complex[] retval = new Complex[coeffs.length];

        //even
        Complex[] even = new Complex[coeffs.length/2];
        int evena =0;
        for(int i=0; i< coeffs.length-1;i=i+2){
            even[evena] = coeffs[i].rootOfUnity(n,k);
            evena++;
        }

        //odd
        Complex[] odd = new Complex[coeffs.length/2];
        int odda =0;
        for(int i=1; i<coeffs.length;i=i+2){
            odd[odda] = coeffs[i].rootOfUnity(n,k);
            odda++;
        }
        ...
        for(int i =0; i< retval.length-1;i++){
            if(i%2==0) retval[i] =  even[evene++];
            else retval[i] =  odd[odde++];
        }
